...
public class LicenceTypeSpinnerAdapter extends BaseAdapter{

    Activity activity;
    LayoutInflater layoutInflater;
    List<LicenceTypeList> lists = new ArrayList<>();

    public LicenceTypeSpinnerAdapter(Activity activity, List<LicenceTypeList> lists) {
    ...
    public View getView(int position, View view, ViewGroup parent) {
        ...
        if(position==0){
            textView.setText("Select License Type");
            textView.setTextColor(ContextCompat.getColor(activity,R.color.grey));
        }else {
            LicenceTypeList list = lists.get(position-1);
            view.setTag(list);
