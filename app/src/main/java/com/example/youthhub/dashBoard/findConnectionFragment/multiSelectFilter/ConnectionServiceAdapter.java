...
public class ConnectionServiceAdapter extends RecyclerView.Adapter<ConnectionServiceAdapter.MyViewHolder> implements View.OnClickListener {

    private Activity activity;
    private List<Service> lists;
    private OnViewClickListener onViewClickListener;
    private List<Service> selectedLists;

    public void addAll(List<Service> lists, List<Service> selectedLists) {
        this.lists = lists;
        this.selectedLists = selectedLists;
        notifyDataSetChanged();
    }

    ConnectionServiceAdapter(Activity activity) {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        Service list = lists.get(position);
        holder.tagsName.setText(list.getOgcName());
        ...
        for (int i=0;i<selectedLists.size();i++){
            if(list.getOgcCategoryId().equals(selectedLists.get(i).getOgcCategoryId())){
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.check_view) {
            ...
            Service singleList = (Service) v.getTag(R.id.data_tag);
    ...
    public interface OnViewClickListener {
        void OnViewItemClick(Service singleList);
    }
