...
public class ConnectionRegionAdapter extends RecyclerView.Adapter<ConnectionRegionAdapter.MyViewHolder> implements View.OnClickListener {

    private Activity activity;
    private List<Region> lists;
    private OnViewClickListener onViewClickListener;
    private List<Region> selectedLists;

    public void addAll(List<Region> lists, List<Region> selectedLists) {
        this.lists = lists;
        this.selectedLists = selectedLists;
        notifyDataSetChanged();
    }

    ConnectionRegionAdapter(Activity activity) {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        Region list = lists.get(position);
        holder.tagsName.setText(list.getReName());
        ...
        for (int i=0;i<selectedLists.size();i++){
            if(list.getReRegionId().equals(selectedLists.get(i).getReRegionId())){
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.check_view) {
            ...
            Region singleList = (Region) v.getTag(R.id.data_tag);
    ...
    public interface OnViewClickListener {
        void OnViewItemClick(Region singleList);
    }
