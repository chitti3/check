...
public class ConnectionWishListAdapter extends RecyclerView.Adapter<ConnectionWishListAdapter.MyViewHolder> implements View.OnClickListener {

    private Activity activity;
    private List<Wishlist> lists;
    private OnViewClickListener onViewClickListener;
    private List<Wishlist> selectedLists;

    public void addAll(List<Wishlist> lists, List<Wishlist> selectedLists) {
        this.lists = lists;
        this.selectedLists = selectedLists;
        notifyDataSetChanged();
    }

    ConnectionWishListAdapter(Activity activity) {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        ...
        holder.tagsName.setText(list.getWitName());
        ...
        for (int i=0;i<selectedLists.size();i++){
            if(list.getWitTagId().equals(selectedLists.get(i).getWitTagId())){
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.check_view) {
            ...
            Wishlist singleList = (Wishlist) v.getTag(R.id.data_tag);
    ...
    public interface OnViewClickListener {
        void OnViewItemClick(Wishlist singleList);
    }
